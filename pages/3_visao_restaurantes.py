...
def order_graph_city(dataset):
    ...
    return fig
def avg_std_festival(dataset,festival,op):
    ''''Esta função cálcula o desvio padrão e o tempo médio das entregas no festival
    
        Input: 
            - dataframe
            - Tipo de operação: 
                1. avg_time: calcula o tempo médio
                2. std_time: calcula o desvio padrão
                3. Festival: Yes  ou No
    ...
    df1.columns = ['avg_time', 'std_time']
    df1 = df1.reset_index()
    metrica = df1.loc[df1['Festival'] == festival, op]
    return metrica
...
with st.container():
    ...
    with col2:
        ...
        metrica = avg_std_festival(dataset,festival='Yes', op='avg_time')
        col2.metric('Tempo médio - Festival',np.round(metrica),2)
    with col3:
        metrica = avg_std_festival(dataset, festival='No', op='avg_time')
        col3.metric('Tempo médio sem Festival',np.round(metrica),2)

with st.container():
    st.markdown('### Distribuição de entrega por cidade')
    fig = order_graph_city(dataset)
    st.plotly_chart(fig, use_container_width=True)
with st.container():
    col1, col2 = st.columns(2)
    with col1:

        fig = avg_distance(dataset)
        col1.plotly_chart(fig, use_container_with=True,)

    with col2:
        fig = avg_city_traffic(dataset)
